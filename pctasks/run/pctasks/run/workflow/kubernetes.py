...
def submit_workflow(workflow_submit_message: WorkflowSubmitMessage, namespace="tasks"):
    ...
    for job_name, job in workflow_submit_message.workflow.definition.jobs.items():
        for task in job.tasks:
            logger.info("Preparing task %s - %s", job_name, task.id)
            input_uri = prepare_task(task, workflow_submit_message.run_id, job_name)

            # Build the stuff
            # XXX: encode the task ID in the name of the object
            # Make sure to escape properly. Kinda hard.
            deployment = build_streaming_deployment(task, input_uri, azure_credentials)
...
def get_azure_credentials() -> dict[str, str]:
    # TODO: this is probably in pctasks somewhere
    kv = azure.keyvault.secrets.SecretClient(
        "https://kv-pctaskstest-staging.vault.azure.net/",
        credential=azure.identity.DefaultAzureCredential(),
    )
    azure_credentials = {
        "AZURE_CLIENT_ID": kv.get_secret("task-client-id").value,
        "AZURE_CLIENT_SECRET": kv.get_secret("task-client-secret").value,
        "AZURE_TENANT_ID": kv.get_secret("task-tenant-id").value,
    }
...
def build_streaming_deployment(
    ...
    ----------
    task_definition : TaskDefinition
        The pctasks TaskDefinition. This should be a steraming-style task.
    input_uri: str
    ...
    azure_credentials: dict
        The mapping of environment variables to values with the Service Principal's
        credentials. This Service Principal is used to access the Azure Storage
        Queue and Cosmos DB container.
    ...
    deployment
        ...
        single container.
    """
    env = [V1EnvVar(name=k, value=v) for k, v in azure_credentials.items()]
...
def create_or_update(object, api, **kwargs):
    """
