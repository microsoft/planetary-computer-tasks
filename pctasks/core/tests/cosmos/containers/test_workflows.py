...
def test_workflow_runs_pagination():
    ...
    with temp_cosmosdb_if_emulator() as db:
        ...
        with WorkflowsContainer(WorkflowRecord, db=db) as workflows:
            workflows.put(workflow)
            assert workflows.get(workflow_id, partition_key=workflow_id)

            with WorkflowsContainer(WorkflowRunRecord, db=db) as workflow_runs:
                ...
                def fetch_workflow() -> WorkflowRecord:
                    fetched_workflow = workflows.get(
                        workflow_id, partition_key=workflow_id
                    )
